...
// Create app/learn/ directory
fs.mkdirSync('app/learn', { recursive: true });
...
fs.writeFileSync('app/learn/page.jsx', page, 'utf8');
console.log('app/learn/page.jsx written.');

// ── Patch Resources page to add Play Games button under Learning at Home ──
const resourcesPath = 'app/resources/page.jsx';
if (fs.existsSync(resourcesPath)) {
  let r = fs.readFileSync(resourcesPath, 'utf8');

  // Find the Learning at Home section heading and inject a game card after it
  const target = `{t('resources.learning')}`;
  const inject = `{t('resources.learning')}
                  </h3>
                  {/* Learning Games card */}
                  <a href="/learn" style={{
                    display:'flex', alignItems:'center', gap:16,
                    padding:'18px 22px', borderRadius:20, marginTop:16, marginBottom:4,
                    background:'linear-gradient(135deg,rgba(75,163,227,0.08),rgba(34,197,94,0.06))',
                    border:'2px solid rgba(75,163,227,0.2)',
                    textDecoration:'none', transition:'transform 0.15s, box-shadow 0.15s',
                    boxShadow:'0 2px 12px rgba(0,0,0,0.04)',
                  }}>
                    <div style={{fontSize:'2rem',flexShrink:0}}>🎮</div>
                    <div style={{flex:1}}>
                      <div style={{fontFamily:'Fredoka',fontSize:'1.1rem',color:'#1B2D5B',lineHeight:1.2}}>
                        {lang==='es'?'Juegos de Aprendizaje':'Learning Games'}
                      </div>
                      <div style={{fontFamily:'DM Sans',fontSize:'0.78rem',color:'#9CA3AF',marginTop:2}}>
                        {lang==='es'?'ABC · Colores · Contar':'ABC · Colors · Counting'}
                      </div>
                    </div>
                    <div style={{fontFamily:'Fredoka',fontSize:'0.85rem',fontWeight:700,color:'#4BA3E3',flexShrink:0}}>
                      {lang==='es'?'Jugar ▶':'Play ▶'}
                    </div>
                  </a>
                  {/* end learning games card — dummy close tag removed below */}
                  <span style={{display:'none'}}`;

  if (r.includes(target)) {
    r = r.replace(target, inject);
    fs.writeFileSync(resourcesPath, r, 'utf8');
    console.log('Resources page patched with Learning Games card.');
  } else {
    console.warn('Resources learning target not found — add link manually.');
  }
} else {
  console.warn('Resources page not found — skipping patch.');
}

console.log('\nDone. Run: npm run dev and visit /learn');
