...
console.log('\n========================================');
console.log('  Rebuilding LearnMoreModal (Complete)');
console.log('========================================\n');
...
const features = [
  { emoji: '\u{1F4DA}', title: 'Creative Curriculum', text: 'Research-backed learning through play and hands-on discovery.', color: '#3B82F6' },
  { emoji: '\u{1F469}\u{200D}\u{1F3EB}', title: 'NJ Certified Teachers', text: 'Every classroom led by a state-certified early childhood educator.', color: '#10B981' },
  { emoji: '\u{1F91D}', title: 'Family Partnership', text: 'Bilingual family liaisons connect you with community resources.', color: '#F59E0B' },
  { emoji: '\u{1F308}', title: 'Two Locations', text: '540 N. Olden Ave in Trenton and 1040 Spruce St in Lawrence.', color: '#EF4444' },
  { emoji: '\u{1F3A8}', title: 'Enrichment Programs', text: 'Music, art, physical education, and technology in daily learning.', color: '#8B5CF6' },
  { emoji: '\u{1F30D}', title: 'Bilingual Support', text: 'English and Spanish-speaking staff ensuring every family feels welcome.', color: '#0EA5E9' },
];
      ...
      <div style={{
        ...
        <div onClick={e => e.stopPropagation()} style={{
          ...
          <div style={{ padding: '40px 36px 36px', clear: 'both' }}>
            <h2 style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 28, fontWeight: 700, color: '#1a3a2a', margin: '0 0 8px', textAlign: 'center' }}>
            ...
            {lightbox === -1 ? (
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16 }}>
                  <button onClick={() => setLightbox(lightbox > 0 ? lightbox - 1 : 5)} style={{ width: 36, height: 36, borderRadius: '50%', border: '1px solid #e8efe9', background: '#fff', cursor: 'pointer', fontSize: 16, color: '#2e7d52', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
                  ...
                  >Back to Grid</button>
                  <button onClick={() => setLightbox(lightbox < 5 ? lightbox + 1 : 0)} style={{ width: 36, height: 36, borderRadius: '50%', border: '1px solid #e8efe9', background: '#fff', cursor: 'pointer', fontSize: 16, color: '#2e7d52', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
                    onMouseEnter={e => { e.currentTarget.style.background = '#f0faf4'; e.currentTarget.style.borderColor = '#48a870'; }}
                    onMouseLeave={e => { e.currentTarget.style.background = '#fff'; e.currentTarget.style.borderColor = '#e8efe9'; }}
                  >{'>'}</button>
                </div>
...
fs.writeFileSync('app/components/LearnMoreModal.jsx', content, 'utf8');
console.log('  CREATED: app/components/LearnMoreModal.jsx (complete rewrite)');
console.log('\n  Features:');
console.log('  - First popup: Logo, title, 6 feature cards with hover effects');
console.log('  - "Peek Inside Our Classrooms" button at bottom');
console.log('  - Second popup: Gallery grid with lightbox navigation');
console.log('  - Both have blur backdrops, spring animations, shimmer bars');
