...
const labelEsStyle = { display: "block", fontSize: 11, fontWeight: 400, color: "#9CA3AF", marginTop: 1 };
...
const inputStyle = {
  ...
  color: "#1F2937", background: "#FFFFFF", outline: "none", boxSizing: "border-box",
};
const errStyle = { display: "block", fontSize: 11, color: "#DC2626", marginTop: 4, fontWeight: 500 };
  ...
  return (
    <div style={{ fontFamily: "'Nunito', 'Segoe UI', system-ui, sans-serif", color: c.gray800, minHeight: "100vh", background: c.cream }}>
      ...
      <div style={{
        ...
        padding: "140px 24px 56px", textAlign: "center", position: "relative", overflow: "hidden",
      }}>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", position: "relative", zIndex: 1 }}>
          <h1 style={{ fontSize: "clamp(28px, 5vw, 40px)", fontWeight: 800, color: c.white, margin: "0 0 10px", letterSpacing: "-0.5px", lineHeight: 1.2 }}>
            Forms &amp; Resources
          </h1>
          ...
          <p style={{ fontSize: "clamp(13px, 2vw, 15px)", color: "rgba(255,255,255,0.55)", margin: "14px auto 0", maxWidth: 520, lineHeight: 1.6 }}>
            Download, print, sign, and securely upload your enrollment forms.<br />
      ...
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "36px 20px 80px" }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 13, color: c.navy, lineHeight: 1.6 }}>
            <strong>How it works:</strong> Download the form, print and sign it, then click <strong>&quot;Upload Signed Form&quot;</strong> to submit securely. You will get an email confirmation.<br />
            <span style={{ color: c.gray500 }}>
              Descargue el formulario, imprima y firme, luego haga clic en &quot;Cargar Formulario&quot; para enviarlo.
            </span>
          </div>
        </div>
              ...
              <div style={{ padding: "24px 24px 20px" }}>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 18, flexWrap: "wrap", alignItems: "center" }}>
                  {form.files.map((file, i) => (
                    <a key={i} href={file.href} download target="_blank" rel="noopener noreferrer" style={{
                      display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 18px", borderRadius: 8,
                      background: c.navy, color: c.white, fontSize: 13, fontWeight: 600, textDecoration: "none",
                      fontFamily: "inherit", cursor: "pointer", transition: "all 0.2s ease", whiteSpace: "nowrap",
                    }}
                    onMouseEnter={(e) => { e.currentTarget.style.opacity = "0.88"; }}
                    onMouseLeave={(e) => { e.currentTarget.style.opacity = "1"; }}
                    >⬇ Download {file.label}</a>
                  ))}
                  <div style={{ width: 1, height: 28, background: c.gray200, margin: "0 4px" }} />
                  <button onClick={() => toggleUpload(form.id)} style={{
                    display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 18px", borderRadius: 8,
                    background: expandedForm === form.id ? c.gold : c.white, color: c.navy,
                    border: `2px solid ${c.gold}`, fontSize: 13, fontWeight: 600, fontFamily: "inherit",
                    cursor: "pointer", transition: "all 0.2s ease", whiteSpace: "nowrap",
                  }}>📤 {expandedForm === form.id ? "Close" : "Upload Signed Form"}</button>
                </div>
              </div>

              {expandedForm === form.id && (
                <div ref={(el) => (uploadRefs.current[form.id] = el)} style={{ borderTop: `1px solid ${c.gray200}`, padding: "24px", background: c.gray50 }}>
                  <div style={{ background: c.greenLight, border: "1px solid #BBF7D0", borderRadius: 8, padding: "10px 14px", marginBottom: 20, display: "flex", gap: 8, alignItems: "center" }}>
                    <span style={{ fontSize: 15 }}>🔒</span>
                    <span style={{ fontSize: 12, color: c.gray700, lineHeight: 1.5 }}>
                      <strong>Secure submission</strong> — files go directly to Laura Spelman Preschool Academy. You will receive email confirmation.
                    </span>
                  </div>

                  {uploadState === "success" && (
                    <div style={{ background: c.white, border: `2px solid ${c.green}`, borderRadius: 12, padding: "36px 24px", textAlign: "center" }}>
                      <div style={{ width: 56, height: 56, borderRadius: "50%", background: c.greenLight, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 28 }}>✓</div>
                      <h3 style={{ fontSize: 18, fontWeight: 700, color: c.green, margin: "0 0 6px" }}>Submitted Successfully!</h3>
                      <p style={{ fontSize: 13, color: c.gold, fontWeight: 600, margin: "0 0 6px" }}>¡Enviado Exitosamente!</p>
                      <p style={{ fontSize: 13, color: c.gray500, margin: "0 0 16px" }}>Confirmation sent to your email.</p>
                      <button onClick={() => { setUploadState("idle"); setFiles([]); setErrors({}); setFormData({ parentName: "", parentEmail: "", childName: "", phone: "", notes: "" }); }}
                        style={{ padding: "10px 24px", borderRadius: 8, background: c.navy, color: c.white, border: "none", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
                  ...
                  {uploadState === "error" && (
                    <div style={{ background: c.redLight, border: `2px solid ${c.red}`, borderRadius: 12, padding: "24px", textAlign: "center", marginBottom: 16 }}>
                      <p style={{ fontSize: 14, fontWeight: 700, color: c.red, margin: "0 0 6px" }}>Error — Please try again</p>
                      <button onClick={() => setUploadState("idle")}
                        style={{ padding: "8px 20px", borderRadius: 8, background: c.navy, color: c.white, border: "none", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", marginTop: 8 }}>
                        Retry / Reintentar
                      </button>
                    </div>
                  )}
                  ...
                  {(uploadState === "idle" || uploadState === "uploading") && (
                    <div>
                      <h4 style={{ fontSize: 15, fontWeight: 700, color: c.navy, margin: "0 0 4px" }}>Uploading: {form.title}</h4>
                      <p style={{ fontSize: 12, color: c.gold, fontWeight: 500, margin: "0 0 18px" }}>Cargando: {form.titleEs}</p>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 14 }}>
                        <div>
                          <label style={labelStyle}>Parent / Guardian Name <span style={reqStyle}>*</span><span style={labelEsStyle}>Nombre del Padre / Tutor</span></label>
                          <input type="text" value={formData.parentName} onChange={(e) => setFormData((p) => ({ ...p, parentName: e.target.value }))} placeholder="Full name / Nombre completo" style={{ ...inputStyle, borderColor: errors.parentName ? c.red : c.gray200 }} />
                          {errors.parentName && <span style={errStyle}>{errors.parentName}</span>}
                        </div>
                        <div>
                          <label style={labelStyle}>Your Email <span style={reqStyle}>*</span><span style={labelEsStyle}>Su Correo Electrónico</span></label>
                          <input type="email" value={formData.parentEmail} onChange={(e) => setFormData((p) => ({ ...p, parentEmail: e.target.value }))} placeholder="email@example.com" style={{ ...inputStyle, borderColor: errors.parentEmail ? c.red : c.gray200 }} />
                          {errors.parentEmail && <span style={errStyle}>{errors.parentEmail}</span>}
                        </div>
                        <div>
                          <label style={labelStyle}>Child&apos;s Name <span style={reqStyle}>*</span><span style={labelEsStyle}>Nombre del Niño/a</span></label>
                          <input type="text" value={formData.childName} onChange={(e) => setFormData((p) => ({ ...p, childName: e.target.value }))} placeholder="Child's full name" style={{ ...inputStyle, borderColor: errors.childName ? c.red : c.gray200 }} />
                          {errors.childName && <span style={errStyle}>{errors.childName}</span>}
                        </div>
                        <div>
                          <label style={labelStyle}>Phone <span style={labelEsStyle}>Teléfono</span></label>
                      ...
                      <div style={{ marginTop: 16 }}>
                        <label style={labelStyle}>Attach Signed Form(s) <span style={reqStyle}>*</span><span style={labelEsStyle}>Adjuntar Formulario(s) Firmado(s)</span></label>
                        <div onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()}
                          style={{ border: `2px dashed ${dragActive ? c.blue : errors.files ? c.red : c.gray300}`, borderRadius: 10, padding: "24px 16px", textAlign: "center", cursor: "pointer", background: dragActive ? c.blueLight : c.white, transition: "all 0.2s ease" }}>
                          ...
                          <p style={{ fontSize: 13, fontWeight: 600, color: c.gray700, margin: "0 0 4px" }}>{dragActive ? "Drop files here" : "Click or drag files here"}</p>
                          <p style={{ fontSize: 11, color: c.gray400, margin: 0 }}>PDF, JPG, PNG, HEIC · Max 10MB · Up to 5 files</p>
                        </div>
                        {errors.files && <span style={errStyle}>{errors.files}</span>}
                        {files.length > 0 && (
                          <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
                            {files.map((file, i) => (
                              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", background: c.white, borderRadius: 6, border: `1px solid ${c.gray200}` }}>
                                <span style={{ fontSize: 14 }}>📄</span>
                                <span style={{ flex: 1, fontSize: 12, fontWeight: 500, color: c.gray700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{file.name}</span>
                                ...
                                <button type="button" onClick={(e) => { e.stopPropagation(); removeFile(i); }}
                                  style={{ width: 20, height: 20, borderRadius: 4, border: "none", background: c.redLight, color: c.red, cursor: "pointer", fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>✕</button>
                              </div>
                            ))}
                          </div>
                        )}
                      ...
                      <div style={{ marginTop: 14 }}>
                        <label style={labelStyle}>Notes <span style={labelEsStyle}>Notas Adicionales</span></label>
                      ...
                      <div style={{ marginTop: 20, textAlign: "center" }}>
                        <button onClick={() => handleSubmit(form.uploadLabel)} disabled={uploadState === "uploading"}
                          ...
                          {uploadState === "uploading" ? "⏳ Submitting..." : "📤 Submit Form / Enviar Formulario"}
                        </button>
                        <p style={{ fontSize: 10, color: c.gray400, marginTop: 8 }}>🔒 Encrypted · Sent directly to LSPA</p>
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div style={{ marginTop: 32, padding: "18px 20px", borderRadius: 10, background: c.white, border: `1px solid ${c.gray200}`, textAlign: "center" }}>
          <p style={{ fontSize: 13, color: c.gray500, margin: 0, lineHeight: 1.6 }}>
            Having trouble? Email forms directly to{" "}
            <a href="mailto:lauraspelmanacademy@verizon.net" style={{ color: c.blue, fontWeight: 600 }}>lauraspelmanacademy@verizon.net</a>{" "}
            or drop them off at either campus.<br />
            <span style={{ color: c.gray400 }}>¿Problemas? Envíe por correo electrónico o entréguelos en persona.</span>
          </p>
        </div>
      </div>
    </div>
